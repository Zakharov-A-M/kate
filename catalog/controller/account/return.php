...
class ControllerAccountReturn extends Controller
{
    ...
    public function index()
    {
        ...
        $data['returns'] = [];
        $returnTotal = $this->model_account_return->getTotalReturns();
        ...
        $data['results'] = sprintf(
            ...
            ($returnTotal) ?
                (($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit')) + 1 :
                0,
            (
                (($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit')) >
                ($returnTotal - $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit'))) ?
                $returnTotal : (
                (($page - 1) * $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit')) +
                $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit')
            ),
            $returnTotal,
            ceil($returnTotal / $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit'))
        );
    ...
    public function info()
    {
        ...
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('account/return/info', 'return_id=' . $returnId);
        ...
        if ($returnInfo) {
            ...
            $data['breadcrumbs'][] = [
                ...
                'href' => $this->url->link(
                    'account/return/info',
                    'return_id=' . $this->request->get['return_id'] . $url
                )
            ];

            $data['return_id'] = $returnInfo['return_id'];
            $data['order_id'] = $returnInfo['order_id'];
            $data['date_ordered'] = date(
                $this->language->get('date_format_short'),
                strtotime($returnInfo['date_ordered'])
            );
            $data['date_added'] = date($this->language->get('date_format_short'), strtotime($returnInfo['date_added']));
            $data['firstname'] = $returnInfo['firstname'];
            $data['lastname'] = $returnInfo['lastname'];
            $data['email'] = $returnInfo['email'];
            $data['telephone'] = $returnInfo['telephone'];
            $data['product'] = $returnInfo['product'];
            $data['model'] = $returnInfo['model'];
            $data['quantity'] = $returnInfo['quantity'];
            $data['reason'] = $returnInfo['reason'];
            $data['opened'] = $returnInfo['opened'] ?
                $this->language->get('text_yes') :
                $this->language->get('text_no');
            $data['comment'] = nl2br($returnInfo['comment']);
            $data['action'] = $returnInfo['action'];
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('text_return'),
                'href' => $this->url->link('account/return/info', 'return_id=' . $returnId . $url)
            );
    ...
    public function add()
    {
        ...
        if (isset($this->request->get['product_id'])) {
            $productInfo = $this->model_catalog_product->getProduct($this->request->get['product_id']);
        }
        ...
        if (isset($this->request->post['date_ordered'])) {
            ...
            $data['date_ordered'] = date('Y-m-d', strtotime($orderInfo['date_added']));
        } else {
            $data['date_ordered'] = '';
        }
        ...
        if ($this->config->get('captcha_' . $this->config->get('config_captcha') . '_status') &&
            in_array('return', (array)$this->config->get('config_captcha_page'))
        ) {
            ...
            if ($informationInfo) {
                $data['text_agree'] = sprintf(
                    ...
                    $this->url->link(
                        ...
                        'information_id=' . $this->config->get('config_return_id')
                    ),
    ...
    protected function validate()
    {
        ...
        if ((utf8_strlen($this->request->post['product']) < 1) ||
            (utf8_strlen($this->request->post['product']) > 255)
        ) {
    ...
    /**
     * Viewed page success return product
     */
