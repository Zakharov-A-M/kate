...
class ControllerStartupStartup extends Controller {
	public function index() {
        ...
        $url = isset($this->request->server['HTTPS']) ? HTTPS_SERVER : HTTP_SERVER;
		...
		// Language Detection
		if (empty($code)) {
            $code = $this->model_localisation_country->getLanguage($url);
		}
		...
		if (isset($this->session->data['customer']) && isset($this->session->data['customer']['customer_group_id'])) {
			...
			$this->config->set('config_customer_id', $this->session->data['customer']['customer_id']);
		} elseif ($this->customer->isLogged()) {
			// Logged in customers
			$this->config->set('config_customer_group_id', $this->customer->getGroupId());
			if ($this->customer->getPartner()) {
                $this->config->set('config_customer_id', $this->customer->getId());
            }
		...
		if (!array_key_exists($code, $currencies)) {
            $code = $this->model_localisation_country->getCurrency($url);
		}
        ...
        if (isset($this->session->data['country'])) {
            $country = $this->session->data['country'];
        } else {
            $country = $this->model_localisation_country->getCountryIso($url);
            $this->session->data['country'] = $country;
        }

        if (!isset($this->request->cookie['country']) || ($this->session->data['country'] != $this->request->cookie['country'])) {
            setcookie('country', $country, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
        }

        $this->config->set('config_current_country', $country);
