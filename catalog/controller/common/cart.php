...
class ControllerCommonCart extends Controller
{
	...
	public function index()
    {
		...
		foreach ($this->cart->getProducts() as $product) {
			if ($product['image']) {
               // $product['image'] = str_replace('catalog/', 'catalog/1c_Images/', $product['image']);
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
                ...
                $total = $this->currency->format($product['price'] * $product['quantity'], $this->session->data['currency'], true);
			} else {
				$price = false;
				$total = false;
			}
