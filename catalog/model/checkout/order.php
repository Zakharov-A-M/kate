...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
                ...
                'GUID'                    => !empty($order_query->row['GUID']) ? $order_query->row['GUID'] : '',
			);
		} else {
			return false;
		}
     ...
     * @param $guid
     */
	public function setOrderGuid($order_id, $guid)
    {
        $this->db->query("UPDATE " . DB_PREFIX . "order SET GUID = '" . $this->db->escape((string)$guid) . "' WHERE order_id = '" . (int)$order_id . "'");
    }
     ...
     * @throws Exception
     */
	public function sendOrder1C($id, $data)
    {
        $products = [];
        ...
        if (isset($data['products'])) {
            ...
            foreach ($data['products'] as $product) {
                $productInfo = $this->model_catalog_product->getProduct((int)$product['product_id']);
                ...
                if (empty($countCurrentCountry) || $countCurrentCountry < $product['quantity']) {
                    ...
                    if (!empty($stockroomsAttached)) {
                        $stockroomProduct = $this->getCountDeliveryInStockroomAttached(
                            $countCurrentCountry,
                            $stockroomsAttached,
                            $product
                        );
                ...
                array_push($products, [
                    'GUID_Nomenclature'   => $productInfo['GUID'],
                    'GUID_Characteristic' => $productInfo['GUID_Characteristic'],
                    ...
                    'discount'  => ($this->currency->convert($productInfo['special'], $productInfo['currency'], $this->session->data['currency']) > 0) ?
                        (string)round($this->currency->convert($productInfo['special'], $productInfo['currency'], $this->session->data['currency']), 2) :
                        (string)round($this->currency->convert($productInfo['price'], $productInfo['currency'], $this->session->data['currency']), 2),
                    'amount'    => (string)$product['quantity'],
                    'stockroom' => !empty($stockroomProduct) ? $stockroomProduct : ''
                ]);
            }
        }
        ...
        $array = [
            'Invoices' => [[
                'Number' => (string)$id,
                'Date' => date("YmdHis"),
                'country' =>  $this->model_localisation_country->getCountryFromIso($this->config->get('config_current_country')),
                'UserType' =>  ((int)$this->config->get('config_customer_group_id') != 3) ? 'FX' : 'UR',
                'currency' =>  (string)$currency['iso'],
                'stockroomGUID' => "",
                'GUID_partner' => $this->customer->getPartner() ? $this->customer->getPartner() : '00000000-0000-0000-0000-000000000000',
                'description' => $this->db->escape((string)$data['comment']),
                'Contact' => [[
                    'firstname' => $this->db->escape((string)$data['firstname']),
                    'lastname' => $this->db->escape((string)$data['lastname']),
                    'patronymic' => $this->db->escape((string)$data['patronymic']),
                    'phone' => $this->db->escape((string)$data['telephone']),
                ...
                'shipping_address' => $this->db->escape((string)$data['shipping_address_1'])
            ]],
        ];
        $array = json_encode($array, JSON_UNESCAPED_UNICODE);
        $this->load->model('restapi/cron');
        $this->model_restapi_cron->addNotification($array, 2, $id);
    }

    /**
     * Get count delivery in stockroom attached
     *
     ...
     * @return array
     */
    public function getCountDeliveryInStockroomAttached(
        int $countCurrentCountry,
        array $stockroomsAttached,
        array $product
    ) {
