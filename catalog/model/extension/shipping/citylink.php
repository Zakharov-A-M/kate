...
class ModelExtensionShippingCitylink extends Model
{
	public function getQuote($address)
    {
		...
		$query = $this->db->query(
		    ...
		    WHERE geo_zone_id = '" . (int)$this->config->get('shipping_citylink_geo_zone_id') . "' 
		    AND country_id = '" . (int)$address['country_id'] . "' 
		    AND (zone_id = '" . (int)$address['zone_id'] . "' 
		    OR zone_id = '0')"
        );

		if (!$this->config->get('shipping_citylink_geo_zone_id') || $query->num_rows) {
			$status = true;
		} else {
			$status = false;
		}
			...
			if ((float)$cost) {
				$quote_data['citylink'] = [
					...
					'text'         => $this->currency->format(
					    $this->tax->calculate(
					        $cost,
                            $this->config->get('shipping_citylink_tax_class_id'),
