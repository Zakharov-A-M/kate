...
class ModelExtensionPaymentYandexMoney extends Model
{
    ...
    public function confirmOrderPayment($orderId, $payment, $statusId)
    {
        ...
        $this->db->query($sql);
        $this->sendConfirmOrder1C($orderId, $payment);
    }

    /**
     * Send confirm payment order to 1C
     *
     * @param int $orderId
     * @param $payment
     */
    public function sendConfirmOrder1C($orderId, $payment)
    {
        $this->load->model('checkout/order');
        $order = $this->model_checkout_order->getOrder($orderId);
        $sql = 'SELECT * FROM ' . DB_PREFIX . 'ya_money_payment WHERE order_id = ' . (int)$orderId;
        $paymentData = $this->db->query($sql);
        ...
        $array = [
            'data' => [
                ...
                'payment_id' => $paymentData->row['payment_id'],
                'total' => $paymentData->row['amount'],
                'currency' => $paymentData->row['currency'],
                'date' => date("YmdHis"),
            ]
        ];
        ...
        $this->model_restapi_cron->addNotification($array, 3, $orderId);

        $this->log('debug', 'Confirm order payment' . $orderId . 'paymentNumber' . $payment->getId() . 'payment_id' . $paymentData->row['payment_id']);
    }
