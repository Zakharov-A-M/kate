...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (!empty($data['GUID'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET GUID = '" . $this->db->escape($data['GUID']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
        }
    ...
    /**
     * Save customer from 1C
     *
     * @param array $data
     * @return int
     */
	public function addCustomer1C(array $data)
    {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET 
            ...
            "', custom_field = '" .
            $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) .
            "', newsletter = '" . (int) $data['newsletter'] .
            "', salt = '', 
            password = '" . $this->db->escape(password_hash($data['password'], PASSWORD_DEFAULT)) .
            "', status = '1"  .
            "', registred = '" . (int) $data['registred'] .
            "', safe = '" . (int) $data['safe'] .
            "', GUID = '" . $data['GUID'] .
            "', partner_guid = '" . $data['GUIDPartner'] .
            "', date_added = NOW()");
    ...
    {
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape((string)$data['firstname']) . "', patronymic = '" . $this->db->escape((string)$data['patronymic']) . "', lastname = '" . $this->db->escape((string)$data['lastname']) . "', email = '" . $this->db->escape((string)$data['email']) . "', telephone = '" . $this->db->escape((string)$data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
        ...
        if (isset($data['registred'])) {
           $this->db->query("UPDATE " . DB_PREFIX . "customer SET registred = '" . $data['registred'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}

        if (!empty($data['GUIDPartner'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET partner_guid = '" . $data['GUIDPartner'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
        }
    ...
    /**
     * Get count customer form GUID
     *
     * @param $guid
     * @return int
     */
    public function getCountCustomerGUID($guid)
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer c WHERE c.GUID = '" . $guid . "'");

        return (int)$query->row['total'];
    }

    /**
     * Get ID customer form GUID
     *
     * @param $guid
     * @return int
     */
    public function getIDCustomerGUID($guid)
    {
        $query = $this->db->query("SELECT c.customer_id AS customer_id FROM " . DB_PREFIX . "customer c WHERE c.GUID = '" . $guid . "'");

        return (int)$query->row['customer_id'];
    }

    /**
     * Get count customer form partner GUID
     *
     * @param $guid
     * @return int
     */
    public function getCountCustomerPartnerGUID($guid)
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer c WHERE c.partner_guid = '" . $guid . "'");
     ...
     * @return int
     */
    public function getIDCustomerPartnerGUID($guid)
    {
        $query = $this->db->query("SELECT c.customer_id AS customer_id FROM " . DB_PREFIX . "customer c WHERE c.partner_guid = '" . $guid . "'");
     ...
     * @return mixed
     */
    public function getCustomerField($id)
    {
        $query = $this->db->query("SELECT c.custom_field AS custom_field FROM " . DB_PREFIX . "customer c WHERE c.customer_id = '" . $id . "'");
     ...
     * @return int
     */
    public function checkRegisteredUser(int $id)
    {
        $query = $this->db->query("
            SELECT c.registred AS registred
            FROM " . DB_PREFIX . "customer c
            WHERE c.customer_id = '" . $id . "'"
        );

        return (int)$query->row['registred'];
    }
}
