...
class ModelCatalogManufacturer extends Model {
	public function addManufacturer($data) {
        ...
        if (isset($data['manufacturer'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "manufacturer SET manufacturer = '" . $this->db->escape((string)$data['manufacturer']) . "' WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
        }

        if (isset($data['GUID'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "manufacturer SET GUID = '" . $this->db->escape((string)$data['GUID']) . "' WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
        }
     ...
     * @return mixed
     */
    public function getManufacturerByName($manufacturer)
    {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "manufacturer WHERE name Like '" . $manufacturer . "%'");

        return $query->row;
    }

    /**
     * Get count manufacturer by GUID
     *
     * @param $guid
     * @return int
     */
    public function getTotalManufacturerByGUID($guid)
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "manufacturer WHERE GUID = '" . $guid . "'");

        return (int)$query->row['total'];
    }

    /**
     * Get id manufacturer by GUID
     *
     * @param $guid
     * @return int
     */
    public function getManufacturerByGUID($guid)
    {
        $query = $this->db->query("SELECT mf.manufacturer_id AS manufacturer_id FROM " . DB_PREFIX . "manufacturer mf WHERE mf.GUID = '" . $guid . "'");
