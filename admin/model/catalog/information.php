...
class ModelCatalogInformation extends Model {
	...
	public function editInformation($information_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', is_visible = '" . (isset($data['is_visible']) ? (int)$data['is_visible'] : 0). "', image = '" . $this->db->escape($data['image']) . "', date_added = now(), status = '" . (int)$data['status'] . "'  WHERE information_id = '" . (int)$information_id . "'");
     ...
     * @return mixed
     */
    public function getAddressInfo(int $languageId)
    {
        if (empty($languageId)) {
            $languageId = 2;
        }
        $query = $this->db->query("SELECT                    
                  ...
                  ac.phone2 AS phone2, 
                  ac.email2 AS email2
                  FROM " . DB_PREFIX . "address_country ac
                  INNER JOIN  " . DB_PREFIX . "country c on c.country_id = ac.country_id   
