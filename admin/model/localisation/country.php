...
class ModelLocalisationCountry extends Model {
    ...
    /**
     * Get countries for contact page
     *
     * @return mixed
     */
    public function getCountriesContact()
    {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "country c INNER JOIN " . DB_PREFIX . "language l ON (c.language_id = l.language_id) INNER JOIN " . DB_PREFIX . "country_description cd ON (c.country_id = cd.country_id) WHERE c.domain IS NOT NULL AND cd.language_id = '".$this->config->get('config_language_id')."'");
        return $query->rows;
    }
