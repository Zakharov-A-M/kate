...
class ModelExtensionPaymentLaybuy extends Model
{
    ...
    {
		$query = $this->db->query("
            SELECT * FROM `" . DB_PREFIX . "laybuy_revise_request` 
		    WHERE `laybuy_revise_request_id` = '" . (int)$id . "'"
        );
    ...
    {
		$query = $this->db->query("
            SELECT * FROM `" . DB_PREFIX . "laybuy_transaction`
		    WHERE `laybuy_transaction_id` = '" . (int)$id . "'"
        );
	...
	public function getTransactionByLayBuyRefId($laybuy_ref_id)
    {
		$query = $this->db->query(
		    "SELECT * FROM `" . DB_PREFIX . "laybuy_transaction`
		     WHERE `laybuy_ref_no` = '" . (int)$laybuy_ref_id . "'"
        );
	...
	public function getTransactionByOrderId($order_id)
    {
		$query = $this->db->query(
		    "SELECT * FROM `" . DB_PREFIX . "laybuy_transaction` 
		     WHERE `order_id` = '" . (int)$order_id . "' ORDER BY `laybuy_ref_no` DESC LIMIT 1"
        );
	...
	public function install()
    {
		...
		$this->model_setting_event->addEvent(
            ...
            'extension/payment/laybuy/deleteOrder'
        );
	}
    ...
    {
		$this->db->query(
		    "UPDATE `" . DB_PREFIX . "order` SET `order_status_id` = '" . (int)$order_status_id . "', 
		    `date_modified` = NOW() 
    ...
    {
		$this->db->query(
		    "UPDATE `" . DB_PREFIX . "laybuy_revise_request` SET 
		    `laybuy_transaction_id` = '" . (int)$data['transaction_id'] . "',
		     ...
		     `suburb` = '" . $this->db->escape((string)$data['suburb']) . "', 
             ...
             `downpayment` = '" . $this->db->escape((string)$data['downpayment']) . "',
             `months` = '" . (int)$data['months'] . "',
             `downpayment_amount` = '" . (float)$data['downpayment_amount'] . "',
             `payment_amounts` = '" . (float)$data['payment_amounts'] . "',
             `first_payment_due` = '" . $this->db->escape((string)$data['first_payment_due']) . "',
             `last_payment_due` = '" . $this->db->escape((string)$data['last_payment_due']) . "',
             `store_id` = '" . (int)$data['store_id'] . "',
             `status` = '" . (int)$data['status'] . "',
             `report` = '" . $this->db->escape((string)$data['report']) . "', 
             `transaction` = '" . (int)$data['transaction'] . "', 
             `paypal_profile_id` = '" . $this->db->escape((string)$data['paypal_profile_id']) . "',
             `laybuy_ref_no` = '" . (int)$data['laybuy_ref_no'] . "',
    ...
    {
		$this->db->query(
		    "UPDATE `" . DB_PREFIX . "laybuy_transaction` SET 
		    `status` = '" . (int)$status . "', 
		    `report` = '" . $this->db->escape($report) . "',
