...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
        ...
        $data['exchange'] = $this->url->link('customer/customer/exchange', 'user_token=' . $this->session->data['user_token'] . $url);
    ...
    {
        $this->load->model('customer/exchange');
        $flag = true;
        while ($flag) {
            $response = $this->model_customer_exchange->getPartners();
            if ($response) {
                foreach ($response as $partner) {
                    $this->model_customer_exchange->parsePartnersResponse($partner);
                }
            } else {
                $flag = false;
            }
	...
	public function orders()
    {
        ...
        $flag = true;
        while ($flag) {
            $response = $this->model_customer_exchange->getOrder();
            if ($response) {
                foreach ($response as $product) {
                    $status = 1;
                    switch ((int)$product['statusID']) {
                        case 0:
                            $result = $this->db->query("SELECT * FROM " . DB_PREFIX . "order WHERE GUID = '" . $product['InvoiseGUID'] . "'");
                            $status = (int)$result->row['order_status_id'];
                            break;
                        case 1:
                            $status = 15;
                            if ($product['payed']) {
                    ...
                    $data = [
                        'score'      => $product['FileName'],
                        'statusId'   => $status,
                        'statusName' => $product['statusName'],
                        'payed'      => $product['payed'],
                    ];
                    $this->model_customer_exchange->updateOrder($product['InvoiseGUID'], $data);
                }
            } else {
                $flag = false;
            }
