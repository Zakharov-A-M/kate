...
class ControllerExtensionPaymentPPExpress extends Controller
{
	...
	public function index()
    {
		...
		$data['button_configure'] = $this->url->link(
		    'extension/module/pp_button/configure',
		...
		$data['search'] = $this->url->link(
		    'extension/payment/pp_express/search',
            'user_token=' . $this->session->data['user_token']
        );
		...
		if (isset($this->request->post['payment_pp_express_colour'])) {
			$data['payment_pp_express_colour'] = str_replace(
			    '#',
                '',
                $this->request->post['payment_pp_express_colour']
            );
		} else {
		...
		if (isset($this->request->post['payment_pp_express_logo']) &&
            is_file(DIR_IMAGE . $this->request->post['payment_pp_express_logo'])
        ) {
		...
		if (isset($this->request->get['retrieve_code']) && isset($this->request->get['merchant_id'])) {
			...
			if (isset($config_response['api_user_name']) &&
                isset($config_response['api_password']) &&
                isset($config_response['signature'])
            ) {
	...
	public function order()
    {
		if ($this->config->get('payment_pp_express_status')) {
			...
			if ($paypal_info) {
				...
				$captured = number_format(
				    $this->model_extension_payment_pp_express->getCapturedTotal(
				        $paypal_info['paypal_order_id']
                    ),
                    2
                );
				...
				$refunded = number_format(
				    $this->model_extension_payment_pp_express->getRefundedTotal(
				        $paypal_info['paypal_order_id']),
                    2
                );
		...
		if ($paypal_info) {
			...
			foreach ($results as $result) {
				$data['transactions'][] = array(
					...
					'view'           => $this->url->link(
					    'extension/payment/pp_express/info',
                        'user_token=' . $this->session->data['user_token'] . '&transaction_id=' . $result['transaction_id']
                    ),
					'refund'         => $this->url->link(
					    'extension/payment/pp_express/refund',
                        'user_token=' . $this->session->data['user_token'] . '&transaction_id=' . $result['transaction_id']
                    ),
					'resend'         => $this->url->link(
					    'extension/payment/pp_express/resend',
                        'user_token=' . $this->session->data['user_token'] . '&paypal_order_transaction_id=' . $result['paypal_order_transaction_id']
                    )
				);
			}
		}
	...
	public function refund()
    {
		...
		//button actions
		$data['action'] = $this->url->link(
		    'extension/payment/pp_express/doRefund', 'user_token=' . $this->session->data['user_token']
        );
		$data['cancel'] = $this->url->link(
		    'extension/payment/pp_express',
            'user_token=' . $this->session->data['user_token']
		...
		$refunded = number_format(
		    $this->model_extension_payment_pp_express->getRefundedTotalByParentId(
		...
		if (isset($this->request->post['transaction_id']) && isset($this->request->post['refund_full'])) {
			...
			if ($this->request->post['refund_full'] == 0 && $this->request->post['amount'] == 0) {
				...
				if ($paypal_order) {
					...
					$current_transaction = $this->model_extension_payment_pp_express->getLocalTransaction(
					...
					if ($result == false) {
						...
						$this->response->redirect(
						    $this->url->link(
						    'sale/order/info',
                            'user_token=' . $this->session->data['user_token'] . '&order_id=' . $paypal_order['order_id']
                            )
                        );
