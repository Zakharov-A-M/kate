...
class ControllerCatalogAttributeGroup extends Controller
{
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_catalog_attribute_group->editAttributeGroup(
                $this->request->get['attribute_group_id'],
                $this->request->post
            );
    ...
    protected function getList()
    {
        ...
        $data['delete'] = $this->url->link(
            'catalog/attribute_group/delete',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        ...
        $attributeGroupTotal = $this->model_catalog_attribute_group->getTotalAttributeGroups();

        $results = $this->model_catalog_attribute_group->getAttributeGroups($filterData);
        ...
        $data['sort_name'] = $this->url->link(
            'catalog/attribute_group',
            'user_token=' . $this->session->data['user_token'] . '&sort=agd.name' . $url
        );
        $data['sort_sort_order'] = $this->url->link(
            'catalog/attribute_group',
            'user_token=' . $this->session->data['user_token'] . '&sort=ag.sort_order' . $url
        );
        ...
        $pagination->total = $attributeGroupTotal;
        ...
        $data['results'] = sprintf(
            ...
            ($attributeGroupTotal) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0,
            (
                (($page - 1) * $this->config->get('config_limit_admin')) >
                ($attributeGroupTotal - $this->config->get('config_limit_admin'))
            ) ? $attributeGroupTotal : (
                (($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')
            ),
            $attributeGroupTotal,
            ceil($attributeGroupTotal / $this->config->get('config_limit_admin'))
        );
    ...
    {
        $data['text_form'] = !isset($this->request->get['attribute_group_id']) ?
        ...
        if (!isset($this->request->get['attribute_group_id'])) {
            ...
            $data['action'] = $this->url->link(
                'catalog/attribute_group/edit',
                'user_token=' . $this->session->data['user_token'] .
                '&attribute_group_id=' . $this->request->get['attribute_group_id'] . $url
            );
        }
        ...
        if (isset($this->request->get['attribute_group_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $attribute_group_info = $this->model_catalog_attribute_group->getAttributeGroup(
                $this->request->get['attribute_group_id']
            );
        }
        ...
        if (isset($this->request->post['attribute_group_description'])) {
            ...
            $data['attribute_group_description'] = $this->model_catalog_attribute_group->getAttributeGroupDescriptions(
                $this->request->get['attribute_group_id']
            );
        } else {
    ...
    protected function validateDelete()
    {
        ...
        foreach ($this->request->post['selected'] as $attributeGroupId) {
            $attributeTotal = $this->model_catalog_attribute->getTotalAttributesByAttributeGroupId($attributeGroupId);

            if ($attributeTotal) {
                $this->error['warning'] = sprintf($this->language->get('error_attribute'), $attributeTotal);
            }
        }
