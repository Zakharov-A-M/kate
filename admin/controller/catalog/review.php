...
class ControllerCatalogReview extends Controller
{
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['filter_product'])) {
                $url .= '&filter_product=' . urlencode(
                        html_entity_decode($this->request->get['filter_product'], ENT_QUOTES, 'UTF-8')
                    );
            }

            if (isset($this->request->get['filter_author'])) {
                $url .= '&filter_author=' . urlencode(
                        html_entity_decode($this->request->get['filter_author'], ENT_QUOTES, 'UTF-8')
                    );
            }
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['filter_author'])) {
                $url .= '&filter_author=' . urlencode(
                        html_entity_decode(
                            $this->request->get['filter_author'],
                            ENT_QUOTES,
                            'UTF-8'
                        )
                    );
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            $data['reviews'][] = [
                ...
                'edit' => $this->url->link(
                    ...
                    '&review_id=' . $result['review_id'] . $url
                )
            ];
        }
        ...
        $data['sort_author'] = $this->url->link(
            'catalog/review',
            'user_token=' . $this->session->data['user_token'] . '&sort=r.author' . $url
        );
        $data['sort_rating'] = $this->url->link(
            'catalog/review',
            'user_token=' . $this->session->data['user_token'] . '&sort=r.rating' . $url
        );
        $data['sort_status'] = $this->url->link(
            'catalog/review',
            'user_token=' . $this->session->data['user_token'] . '&sort=r.status' . $url
        );
        $data['sort_date_added'] = $this->url->link(
            'catalog/review',
            'user_token=' . $this->session->data['user_token'] . '&sort=r.date_added' . $url
        );
    ...
    {
        $data['text_form'] = !isset($this->request->get['review_id']) ?
        ...
        if (!isset($this->request->get['review_id'])) {
            ...
            $data['action'] = $this->url->link(
                'catalog/review/edit',
                'user_token=' . $this->session->data['user_token'] .
                '&review_id=' . $this->request->get['review_id'] . $url
            );
        }
        ...
        if (isset($this->request->post['date_added'])) {
            ...
            $data['date_added'] = ($review_info['date_added'] != '0000-00-00 00:00' ?
                $review_info['date_added'] :
                date('Y-m-d')
            );
        } else {
