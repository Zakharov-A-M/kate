...
class ControllerCatalogRecurring extends Controller
{
    ...
    protected function getList()
    {
        ...
        $data['add'] = $this->url->link(
            'catalog/recurring/add',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        $data['copy'] = $this->url->link(
            'catalog/recurring/copy',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        $data['delete'] = $this->url->link(
            'catalog/recurring/delete',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        ...
        foreach ($results as $result) {
            $data['recurrings'][] = [
                ...
                'edit' => $this->url->link(
                    'catalog/recurring/edit',
                    'user_token=' . $this->session->data['user_token'] .
                    '&recurring_id=' . $result['recurring_id'] . $url
                )
            ];
        }
        ...
        $data['results'] = sprintf(
            ...
            ($recurring_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0,
            (
                (($page - 1) * $this->config->get('config_limit_admin')) >
                ($recurring_total - $this->config->get('config_limit_admin'))
            ) ? $recurring_total : (
                (($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')),
            $recurring_total,
            ceil($recurring_total / $this->config->get('config_limit_admin'))
        );
    ...
    {
        $data['text_form'] = !isset($this->request->get['recurring_id']) ?
        ...
        if (!isset($this->request->get['recurring_id'])) {
            ...
            $data['action'] = $this->url->link(
                'catalog/recurring/edit',
                'user_token=' . $this->session->data['user_token'] .
                '&recurring_id=' . $this->request->get['recurring_id'] . $url
            );
        }
        ...
        if (isset($this->request->post['recurring_description'])) {
            ...
            $data['recurring_description'] = $this->model_catalog_recurring->getRecurringDescription(
                $recurring_info['recurring_id']
            );
        } else {
