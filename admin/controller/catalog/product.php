...
class ControllerCatalogProduct extends Controller
{
    ...
    public function importXML()
    {
        ...
        $data['export_xml'] = $this->url->link(
            'catalog/product/importXML',
            'user_token=' . $this->session->data['user_token'] . $url
        );
    ...
    public function add()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['filter_model'])) {
                $url .= '&filter_model=' . urlencode(
                        html_entity_decode($this->request->get['filter_model'], ENT_QUOTES, 'UTF-8')
                    );
            }
    ...
    protected function getList()
    {
            ...
            foreach ($product_specials as $product_special) {
                if (
                    ...
                    $special = $this->currency->format(
                        ...
                        $result['currency'] ?? $this->config->get('config_currency'),
                        true
                    );
                    break;
                }
        ...
        $data['sort_model'] = $this->url->link(
            'catalog/product',
            'user_token=' . $this->session->data['user_token'] . '&sort=p.model' . $url
        );
        $data['sort_price'] = $this->url->link(
            'catalog/product',
            'user_token=' . $this->session->data['user_token'] . '&sort=p.price' . $url
        );
        $data['sort_quantity'] = $this->url->link(
            'catalog/product',
            'user_token=' . $this->session->data['user_token'] . '&sort=p.quantity' . $url
        );
        $data['sort_status'] = $this->url->link(
            'catalog/product',
            'user_token=' . $this->session->data['user_token'] . '&sort=p.status' . $url
        );
        $data['sort_order'] = $this->url->link(
            'catalog/product',
            'user_token=' . $this->session->data['user_token'] . '&sort=p.sort_order' . $url
        );
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->post['product_description'])) {
            ...
            $data['product_description'] = $this->model_catalog_product->getProductDescriptions(
