...
class ControllerCatalogDownload extends Controller
{
    ...
    protected function getList()
    {
        ...
        $data['add'] = $this->url->link(
            'catalog/download/add',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        $data['delete'] = $this->url->link(
            'catalog/download/delete',
            'user_token=' . $this->session->data['user_token'] . $url
        );
        ...
        foreach ($results as $result) {
            $data['downloads'][] = [
                ...
                'edit' => $this->url->link(
                    'catalog/download/edit',
                    'user_token=' . $this->session->data['user_token'] . '&download_id=' . $result['download_id'] . $url
                )
            ];
        }
        ...
        $data['sort_name'] = $this->url->link(
            'catalog/download',
            'user_token=' . $this->session->data['user_token'] . '&sort=dd.name' . $url
        );
        $data['sort_date_added'] = $this->url->link(
            'catalog/download',
            'user_token=' . $this->session->data['user_token'] . '&sort=d.date_added' . $url
        );
        ...
        $data['results'] = sprintf(
            ...
            ($download_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0,
            (
                (($page - 1) * $this->config->get('config_limit_admin')) >
                ($download_total - $this->config->get('config_limit_admin'))
            ) ? $download_total : (
                (($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')),
            $download_total,
            ceil($download_total / $this->config->get('config_limit_admin'))
        );
    ...
    {
        $data['text_form'] = !isset($this->request->get['download_id']) ?
        ...
        if (!isset($this->request->get['download_id'])) {
            ...
            $data['action'] = $this->url->link(
                'catalog/download/edit',
                'user_token=' . $this->session->data['user_token'] .
                '&download_id=' . $this->request->get['download_id'] . $url
            );
        }
        ...
        if (isset($this->request->post['download_description'])) {
            ...
            $data['download_description'] = $this->model_catalog_download->getDownloadDescriptions(
                $this->request->get['download_id']
            );
        } else {
    ...
    public function report()
    {
        ...
        foreach ($results as $result) {
            ...
            $data['reports'][] = [
                ...
                'filter_ip' => $this->url->link(
                    'customer/customer',
                    'user_token=' . $this->session->data['user_token'] . '&filter_ip=' . $result['ip']
                )
            ];
        }
        ...
        $pagination->limit = 10;
        $pagination->url = $this->url->link(
            'catalog/download/report',
            'user_token=' . $this->session->data['user_token'] . '&download_id=' . $download_id . '&page={page}'
        );
        ...
        $data['results'] = sprintf(
            $this->language->get('text_pagination'),
            ($report_total) ? (($page - 1) * 10) + 1 : 0,
            (
                (($page - 1) * 10) > ($report_total - 10)
            ) ? $report_total : ((($page - 1) * 10) + 10),
            $report_total,
            ceil($report_total / 10)
        );
