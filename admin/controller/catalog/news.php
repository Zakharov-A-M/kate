...
class ControllerCatalogNews extends Controller
{
    ...
    private function getList()
    {
        ...
        foreach ($results as $result) {
            ...
            $data['news'][] = [
                ...
                'selected' => isset($this->request->post['selected']) && in_array(
                        $result['news_id'],
                        $this->request->post['selected']
                    ),
                'edit' => $this->url->link(
                    'catalog/news/edit',
                    'user_token=' . $this->session->data['user_token'] . '&news_id=' . $result['news_id'],
                    true
                )
            ];
        }
        ...
        $data['sort_title'] = $this->url->link(
            'catalog/news',
            'user_token=' . $this->session->data['user_token'] . '&sort=nd.title' . $url,
            true
        );
        $data['sort_date_added'] = $this->url->link(
            'catalog/news',
            'user_token=' . $this->session->data['user_token'] . '&sort=n.date_added' . $url,
            true
        );
        ...
        $data['results'] = sprintf(
            ...
            ($news_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0,
            (
                (($page - 1) * $this->config->get('config_limit_admin')) >
                ($news_total - $this->config->get('config_limit_admin'))
            ) ? $news_total : (
                (($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')),
            $news_total,
            ceil($news_total / $this->config->get('config_limit_admin'))
        );
    ...
    private function getForm()
    {
        ...
        $data['text_form'] = !isset($this->request->get['news_id']) ?
