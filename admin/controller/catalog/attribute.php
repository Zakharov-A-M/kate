...
class ControllerCatalogAttribute extends Controller
{
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            $data['attributes'][] = [
                ...
                'edit' => $this->url->link(
                    ...
                    'user_token=' . $this->session->data['user_token'] .
                    '&attribute_id=' . $result['attribute_id'] . $url
                )
            ];
        }
        ...
        $data['sort_name'] = $this->url->link(
            'catalog/attribute',
            'user_token=' . $this->session->data['user_token'] . '&sort=ad.name' . $url
        );
        $data['sort_attribute_group'] = $this->url->link(
            'catalog/attribute',
            'user_token=' . $this->session->data['user_token'] . '&sort=attribute_group' . $url
        );
        $data['sort_sort_order'] = $this->url->link(
            'catalog/attribute',
            'user_token=' . $this->session->data['user_token'] . '&sort=a.sort_order' . $url
        );
        ...
        $data['results'] = sprintf(
            ...
            ($attribute_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0,
            ((($page - 1) * $this->config->get('config_limit_admin')) >
                ($attribute_total - $this->config->get('config_limit_admin'))) ?
                $attribute_total : (
                (($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')
            ),
            $attribute_total,
            ceil($attribute_total / $this->config->get('config_limit_admin'))
        );
    ...
    {
        $data['text_form'] = !isset($this->request->get['attribute_id']) ?
        ...
        if (!isset($this->request->get['attribute_id'])) {
            ...
            $data['action'] = $this->url->link(
                'catalog/attribute/edit',
                'user_token=' . $this->session->data['user_token'] .
                '&attribute_id=' . $this->request->get['attribute_id'] . $url
            );
        }
        ...
        if (isset($this->request->get['attribute_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $attributeInfo = $this->model_catalog_attribute->getAttribute($this->request->get['attribute_id']);
        }
        ...
        if (isset($this->request->post['attribute_description'])) {
            ...
            $data['attribute_description'] = $this->model_catalog_attribute->getAttributeDescriptions(
        ...
        } elseif (!empty($attributeInfo)) {
            $data['attribute_group_id'] = $attributeInfo['attribute_group_id'];
        } else {
    ...
    protected function validateDelete()
    {
        ...
        foreach ($this->request->post['selected'] as $attribute_id) {
            $productTotal = $this->model_catalog_product->getTotalProductsByAttributeId($attribute_id);

            if ($productTotal) {
                $this->error['warning'] = sprintf($this->language->get('error_product'), $productTotal);
            }
        }
    ...
    public function autocomplete()
    {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_catalog_attribute->getAttributes($filterData);
