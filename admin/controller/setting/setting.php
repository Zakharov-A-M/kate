...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		$data['country_contact'] = $this->model_localisation_country->getCountriesContact();
        ...
        $data['captcha_pages'][] = [
            'text'  => $this->language->get('text_authorization'),
            'value' => 'authorization'
        ];
	...
	protected function validate() {
		...
		foreach ($this->request->post['config_name'] as $key => $config_name) {
            if (!$config_name) {
                $this->error['name'][$key] = $this->language->get('error_name');
            }
		}

        foreach ($this->request->post['config_owner'] as $key => $config_owner) {
            if ((utf8_strlen($config_owner) < 3) || (utf8_strlen($config_owner) > 64)) {
                $this->error['owner'][$key] = $this->language->get('error_owner');
            }
        }

        foreach ($this->request->post['config_address'] as $key => $config_address) {
            if (utf8_strlen($config_address) < 3) {
                var_dump(utf8_strlen($config_address));
                $this->error['address'][$key] = $this->language->get('error_address');
            }
        }

        foreach ($this->request->post['config_email'] as $key => $config_email) {
            if ((utf8_strlen($config_email) > 96) || !filter_var($config_email, FILTER_VALIDATE_EMAIL)) {
                $this->error['email'][$key] = $this->language->get('error_email');
            }
        }

        foreach ($this->request->post['config_email'] as $key => $config_telephone) {
            if ((utf8_strlen($config_telephone) < 3) || (utf8_strlen($config_telephone) > 32)) {
