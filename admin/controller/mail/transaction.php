	...
	public function index($route, $args, $output) {
		...
		if ($customer_info) {
			...
			if ($store_info) {
				...
				$store_name = $this->config->get('config_name')[$this->config->get('config_country_id')];
			}
			...
			$mail->setSubject(sprintf($this->language->get('text_subject'), html_entity_decode($this->config->get('config_name')[$this->config->get('config_country_id')], ENT_QUOTES, 'UTF-8')));
